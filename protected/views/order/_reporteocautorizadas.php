...
//"page":1,"total":3,"records":20,"rows":
    ...
    var _url='<?php echo Yii::app()->createUrl('order/ocautorizadasdata')."&projectid=".$project->id."&budgetid=".$budgetid."&page=1"; ?>';
    ...
    $("#jqGrid").jqGrid({
      ...
      datatype: "json",
      page: 1,
      colNames: ['Folio', 'Partida', 'Subpartida', 'Proveedor', 'Monto', 'Autorizada', 'Recibir', 'Tope Presupuesto'],
      colModel: [
        { name: 'Folio', key: false, width: 30 },
        { name: 'Partida', key: false, width: 100 },
        { name: 'Subpartida', key: false, width: 100 },
        { name: 'Proveedor', key: false, width: 100 },
        { name: 'Monto', key: false, width: 70, search: false, align: 'right', 
                 formatter:'currency', formatoptions: opt},
        { name: 'Autorizada', key: false, width: 25, search: false},
        { name: 'Recibir', key: false, width: 25, search: false},
        { name: 'TopePresupuesto', key: false, width: 50, search: false, align: 'right',
                 formatter:'currency', formatoptions: opt},
      ],
      loadonce: true,
      autowidth: true,
    ...
    $('#jqGrid').jqGrid('navGrid',"#jqGridPager", {                
      ...
      onClickButton: function() {
        window.open("<?php echo Yii::app()->createUrl('order/reporteocautorizadaspdf')."&projectid=".$project->id."&budgetid=".$budgetid."&page=1"; ?>",'_blank');
      }
    });

});

function recibirOrden(orderid){
  bootbox.confirm({
    message: "¿Desea recibir la orden: " + orderid + " ?",
    buttons: {
      confirm: {
        label: 'Si',
    ...
    callback: function (result) {
      if(result){
        $.blockUI({ message: 'Recibiendo orden de compra.' });
        $.ajax({
          type: 'GET',
          async: true,
          contentType: "application/json",
          url: "<?php echo Yii::app()->createUrl('/order/recibir/').'&orderid='; ?>"+orderid,
          success: function(data){
            $.unblockUI();
            $('.recibir'+orderid).each(function(){this.parentElement.innerHTML = '<div style="color: red">Surtida</div>';});
          },error:function(data){
            if(data.status=403){
              $.unblockUI();
              bootbox.alert("Solo puede surtir la orden de compra un usuario \"receptor o administrador\".");
            }
          }
        });
      }
