...
class AjaxController extends RbController {
	...
	public function actionInfoAuthItem() {
		if(isset($_POST['item'])) {
			...
			echo '<span class="authItemLabel">' . Yii::t('RbacuiModule.rbacui',$this->authItemTypeName[$authItem->type]) . ': </span> ' . $authItem->name;
			echo '</div>' . PHP_EOL;
			...
			if(count($parents) == 0) {
				...
				foreach($parents as $name => $item) {
					echo '<div class="authItemLine">';
					echo $name . ' (' . Yii::t('RbacuiModule.rbacui',$this->authItemTypeName[$item->type]) . ')';
					echo '</div>' . PHP_EOL;
				}
			}
			echo '<hr>';
